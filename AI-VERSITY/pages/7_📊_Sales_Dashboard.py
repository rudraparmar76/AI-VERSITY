...
if 'signedin' in st.session_state and st.session_state.signedin:
    ...
    usage_data_path = r'UsageData/dashboard_usage_data.csv'
    if os.path.exists(usage_data_path):
        usage_data = pd.read_csv(usage_data_path)
    else:
        usage_data = pd.DataFrame(columns=['UploadedFile','Timestamp'])
    ...
    with top_left_column:
        ...
        with column_1:
            plot_metric(
                "Total Accounts Receivable",6621280,prefix="$",suffix="",show_graph=True,color_graph="rgba(0,104,201,0.2)",
            )
            plot_gauge(1.86,"#0068C9","%","Current Ratio",3)

        with column_2:
            plot_metric(
                "Total Accounts Payable",1630270,prefix="$",suffix="",show_graph=True,color_graph="rgba(255,43,43,0.2)",
            )
    ...
    with bottom_right_column:
        plot_bottom_right()
    usage_data = usage_data._append({'UploadedFile': uploaded_file.name,'Timestamp': datetime.now()}, ignore_index=True)
